...
function getdata() {
  ...
  $(document).ready(function(){$("#data_btn").val('正在获取，请稍候...'); $.ajax({type: "GET", url: sitepath + '?mod=ajaxget&type=data', data: 'url=' + url, datatype: "script", cache: false, success: function(data){$("body").append(data)}}); $("#data_btn").val('重新获取');$('.btn-gray').removeClass('btn-gray').attr("disabled","");});
}
function checkSubmitButton(){
  if($('#web_ip').val() != ''){
    $('.btn-gray').removeClass('btn-gray').attr("disabled","");
  }
}
