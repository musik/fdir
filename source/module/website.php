...
function get_websites($cate_id = 0, $top_num = 10, $is_pay = false, $is_best = false, $field = 'ctime', $order = 'desc') {
	...
	if (in_array($field, array('instat', 'outstat', 'views','grank', 'brank', 'srank', 'arank'))) $sortby = 'd.web_'.$field;
  if(!$sortby) $sortby = "w.web_ctime";
	...
	while ($row = $DB->fetch_array($query)) {
		...
		$row['web_link'] = get_website_url($row['web_url']);
...
function get_website_rssfeed($cate_id = 0) {
	...
	while ($row = $DB->fetch_array($query)) {
		...
		$row['web_link'] = str_replace('&', '&amp;', get_website_url($row['web_url'], true));
		$results[] = $row;
	}
	unset($row);
	$DB->free_result($query);
